...
def scan_inside_days():
    ...
    all_inside_days_sorted_for_check = sorted(all_inside_days, key=lambda x: x["score"], reverse=True)
...
if __name__ == "__main__":
    ...
    if tier_a_filtered:
        ...
        print("ticker | score | dir   | date       | prev_high | prev_low | close   | ATR10 | best_spread | ema21_slope | slope_norm")
        for r in tier_a_filtered:
            print(
                ...
                f"{r['ema21_slope']:>10} | {r.get('ema21_slope_norm','NA'):>9}"
            )
    else:
